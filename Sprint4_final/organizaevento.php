...
require_once("conecta.php");
	...
	if($conexao)
	{
		...
		$sql = "INSERT INTO TBL_EVENTO (EVE_NOME, EVE_DATA, EVE_HORAINICIO,EVE_LOCAL,EVE_ENDERECO, EVE_NUMERO_ENDERECO, EVE_CIDADE, EVE_ESTADO,EVE_NPROVAS, ID_USER, EVE_RESULTADO)VALUES"

				."('$nomeEvento', STR_TO_DATE('$datepicker', '%d/%m/%Y'), '$inputHoraEvento', '$inputLocal', '$inputEndereco', '$inputNumero', '$inputCidade', '$inputEstado', '$inputQtdeProvas', '$id_user', '$inputResultado')";
...
<?PHP
	...
	$idmod = $linha['ID_USER'];
		if ($id <> $idmod) {
			...
			header("Refresh: 0; logado.php");
		}
		else{
			mysqli_close($conexao);
			header("Refresh: 0; logadomoderador.php");}
?>
